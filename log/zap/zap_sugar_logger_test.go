...
import (
	kitzap "dev.azure.com/vib-lz-devops/B08-DSC-Project-SmartCollection/_git/smartcollection-base-go.git/log/zap"
	"encoding/json"
	"go.uber.org/zap"
...
func TestZapSugarLogger(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.level.String(), func(t *testing.T) {
			...
			gologger := kitzap.NewZapSugarLogger(logger, testCase.level)
			defer func() {
				isPanic := recover() != nil
